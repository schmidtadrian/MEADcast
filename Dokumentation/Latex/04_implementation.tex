...
\section{Router} % (fold)
\label{sec:Router}
This section briefly introduces the fundamentals of the MEADcast router
    implementation in the Linux Kernel.
The MEADcast routing software is developed and tested with the latest stable
    Kernel release version 6.5.2 at the time.
...
This feature can be enabled or disabled at compile-time using the
    \texttt{CONFIG\_IPV6\_MEADCAST} flag.
...
Consequently, we embedded the entry point for MEADcast processing at the end of
    the \texttt{ip6\_forward} method located in
    \texttt{net/ipv6/ipv6\_output.c}.
If a packet contains the routing header extension the \texttt{mdc\_rcv} method 
    located in \texttt{net/ipv6/mdcast.c} gets invoked.
This method performs basic sanity checks on the MEADcast header and then
    invokes \texttt{mdc\_dcv\_rcv} for discovery and \texttt{mdc\_data\_rcv}
    for data packets.
...
Lastly, the \texttt{dst\_output} method is invoked, dispatching the packet to
